...
BDB.RMP<-function(wt, es.source, var.source, N.source, es.target, var.target, N.target, type){
...
BDB.RMP(0.7, -0.6941, 9.354928, 551, -0.3945, 12.36577, 25, "log_exp")
...
###########################################
###########################################
BDB.RMP(0.2, 21, 22.65, 120, 21, 22.65, 60, "normal")
...
weight_range<-do.call(rbind, lapply(1:11, function(i) {
  result=BDB.RMP(0.1*(i-1), -0.6941, 9.354928, 551, -0.3945, 12.36577, 25, "log_exp")
  c(result=result)
}))
...
weight_ss=do.call(rbind, lapply(1:551, function(k){
  ...
  do.call(rbind, lapply(1:11, function(i) {
    result=BDB.RMP(0.1*(i-1), -0.6941, 9.354928, 551, -0.3945, 12.36577, k, "log_exp")
    c(result=result)
  }))
...
power_result <- sapply(1:1000, function(i) { 
   BDB.RMP(0.7, -0.6941, 9.354928, 551, mean(sim[,i]), var(sim[,i]), N.target, "log_exp")["upper.Cri"]})
